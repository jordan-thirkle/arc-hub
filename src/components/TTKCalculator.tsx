...
export function TTKCalculator({ weapon, tier, attachments }: TTKCalculatorProps) {
  ...
  const results = targets.reduce((acc, t) => {
  ...
  return (
    <section className="space-y-2">
      <h2 className="text-[10px] font-mono uppercase tracking-[0.15em] text-secondary font-semibold">
        TTK Analysis
      </h2>
      <div className="grid grid-cols-4 gap-1">
        {targets.map(target => {
          ...
          return (
            ...
            >
              <p className="text-[7px] font-mono uppercase tracking-[0.1em] mb-1"
              ...
              {isHovered ? (
                ...
                <div>
                  <p className="text-xs font-mono font-bold" style={{ color: r.ttk < 1 ? '#4CAF50' : r.ttk < 2 ? '#FFC107' : '#F44336' }}>
                    {r.ttk.toFixed(2)}s
                  </p>
                  <p className="text-[8px] font-mono text-tertiary">
                    {r.shotsToKill} shots
                  ...
                  {target !== 'unshielded' && (
                    <p className="text-[7px] font-mono text-tertiary">
                      HP: {TARGET_SHIELD_HP[target]}
                    </p>
                  )}
                </div>
              )}
