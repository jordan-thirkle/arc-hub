...
function getMsUntilReset(): { total: number; days: number; hours: number; minutes: number } {
  ...
  const next = new Date(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + daysUntil, RESET_HOUR, 0, 0),
  );
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between border-b border-[rgb(var(--border-primary))] pb-3">
        <div>
          ...
          <p className="text-[9px] text-tertiary font-mono mt-0.5">
            Weekly rotating stock — Topside Valuables accepted
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1 max-h-80 overflow-y-auto">
            {ermalAcceptedItems.map(item => (
              <div
                key={item.id}
                className="flex items-center gap-2 px-2 py-1.5 border border-[rgb(var(--border-primary))] bg-surface"
              >
          ...
          <div className="space-y-2">
            {ermal.inventory.map(item => {
              ...
              return (
                <div
                  key={item.itemId}
                  className={`border p-2 ${canAfford ? 'border-accent/60 bg-elevated' : 'border-[rgb(var(--border-primary))] bg-surface'}`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div>
                      ...
                      <p className="text-[8px] font-mono text-tertiary">
                        {item.category} &middot; {target.toLocaleString()}c
                      </p>
                    </div>
