          ...
          <div key={p.id} className="border border-[rgb(var(--border-primary))] bg-surface p-2">
            ...
            <ul className="space-y-0.5">
              {p.changes.slice(0, 3).map((c, i) => (
                <li key={i} className="text-[8px] text-secondary pl-2 border-l border-[rgb(var(--border-primary))]">{c}</li>
              ))}
              {p.changes.length > 3 && (
          ...
          {[...patches].reverse().map(p => {
            ...
            return (
              <div key={p.id} className={`relative pl-6 ${isHighlighted ? 'bg-accent/5 -mx-2 px-2 rounded-sm' : ''}`}>
                <div className={`absolute left-1.5 top-1.5 w-2 h-2 rounded-full border-2 ${
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {isLatest && (
                    <span className="text-[7px] px-1 py-0.5 bg-accent text-page font-mono uppercase tracking-[0.1em]">Latest</span>
                  )}
                ...
                <ul className="space-y-1 mb-1.5">
                  {p.changes.map((c, i) => (
                    <li key={i} className="text-[9px] text-primary pl-3 border-l border-[rgb(var(--border-primary))] leading-tight">{c}</li>
                  ))}
                </ul>
                {weapons.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {weapons.map(w => (
                      <span key={w.id} className="text-[7px] px-1 py-0.5 border border-[rgb(var(--border-primary))] text-tertiary font-mono uppercase">
                        {w.name}
                      </span>
                    ))}
