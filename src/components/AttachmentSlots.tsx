...
export function AttachmentSlots({
  ...
  if (slots.length === 0) {
    return (
      <div className="p-4 border border-[rgb(var(--border-primary))] bg-surface text-center rounded-md">
        <p className="text-xs text-tertiary font-mono uppercase tracking-[0.1em]">No Attachment Slots</p>
  ...
  return (
    <section key={weaponId} className="space-y-2">
      <h3 className="text-[10px] font-mono uppercase tracking-[0.15em] text-secondary font-semibold">{label}</h3>
        ...
        {slots.map(slot => {
          ...
          return (
            <div key={slot} className="relative">
              ...
              {isOpen && (
                <div
                  className="absolute z-20 top-full left-0 right-0 mt-1 border border-[rgb(var(--border-primary))] bg-surface rounded-md shadow-elevated max-h-56 overflow-y-auto min-w-[200px]"
                  role="listbox"
                >
                  <button
                    onClick={() => {
